            ...
            {importMethod === "url" ? (
              <>
                <div className="dialog-field subscription-transport">
                  ...
                  />
                  <small id="subscription-transport-help">Выбор применяется только к этой загрузке. RouteDeck не меняет настройку Windows, не переключается автоматически и не отправляет URL повторно. Прокси-режим принимает только статический числовой loopback и делает CONNECT к закреплённому публичному IP. Прокси видит этот IP и может видеть имя сервера через TLS SNI. Секретные path, query/token, HTTP-заголовки и содержимое скрыты TLS только без перехвата сертификатом из доверенного хранилища Windows. RouteDeck использует это хранилище и не может распознать намеренно доверенный локальный корневой сертификат как враждебный.</small>
                </div>
