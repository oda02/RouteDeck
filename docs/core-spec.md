...
4. The packaged app verifies `sing-box.exe` and `libcronet.dll` against the embedded lock before every launch. Verification and launch are one native, security-reviewed operation that prevents either file from being replaced between hashing and `CreateProcess` (for example, with package-private ACLs, handles held without write/delete sharing, and file-identity revalidation). Hashing a path and later launching that path is insufficient because of the time-of-check/time-of-use race. Missing, extra replacement, or mismatched files produce `Engine integrity check failed`; they are never executed. The repository PowerShell verifier is only a developer/packaging check and does not satisfy this runtime requirement.
