...
4. The packaged app verifies `sing-box.exe` and `libcronet.dll` against the embedded lock before every launch. It copies only those two files from the verified, held package handles into a fresh random per-session directory below the current user's LocalAppData session root. That directory is accepted only on a local fixed NTFS/ReFS volume with persistent ACLs, then sealed with an explicit current-user read/execute/delete plus SYSTEM protected DACL. The runtime hashes the destination, records opened-handle file IDs, owner and security descriptors, rejects reparse points and every extra name, proves that late file creation is denied, and repeats all of those checks immediately before suspended `CreateProcessW`. Exact directory and file handles remain open without write/delete sharing across process creation and for the child lifetime. Missing, extra, replaced, ACL-modified, or mismatched files produce `Engine integrity check failed`; they are never executed. Hashing a path and later launching that path, or merely holding a directory handle, is insufficient. The repository PowerShell verifier remains only a developer/packaging check.
...
6. Preserve upstream licenses/notices in the portable distribution. Engine hash verification does not establish licensing compliance. Release packaging remains blocked until legal review defines a complete, pinned notices/source-compliance manifest that packaging can enforce. Do not call the product or binary an official SagerNet client.
...
- Fetch-only manual-proxy discovery reads the flat `ProxyEnable`/`ProxyServer` settings, as the standard Rust HTTP stack does on Windows. An explicit disabled setting wins over stale WinInet flags; an enabled supported loopback endpoint wins over stale direct-only flags. `ProxySettingsPerUser=0` selects machine settings without consulting user values. Missing user settings may use the existing WinInet fallback; malformed/unreadable settings fail the fetch rather than silently bypassing a configured proxy. Once a proxy is selected, a network failure does not trigger a direct retry. This compatibility reader does not change the separate WinInet ownership/apply/recovery implementation. See Microsoft's [manual proxy values](https://devblogs.microsoft.com/scripting/how-can-i-switch-between-using-a-proxy-server-and-not-using-a-proxy-server/) and [machine-scope policy](https://learn.microsoft.com/en-us/windows-hardware/customize/desktop/unattend/microsoft-windows-ie-clientnetworkprotocolimplementation-hklmproxyenable).
